...
%Another large class of semi-supervised algorithms are formed by manifold regularization methods \citep{Niyogi2013}. Although these have been successful in some applications, they require more domain knowledge to be effectively applied on a particular classification problem and are beyond the scope of the current work.
...
%One outlier is the "EEG eye" dataset, for which the performance does degrade for the projection procedure in the inductive setting. However, while not protecting against all degradation in this setting, its performance in still more robust than the self-learning procedure.

\begin{figure*}
\centering
\includegraphics[scale=0.80]{Figure1.pdf}
...
%This perhaps unusual number of labeled objects is chosen to be able to study the performance with very small numbers of labeled objects, while ensuring that with high probability, for a particular sample from the dataset, the assumptions of Theorem \ref{th:robustness} still hold. This dependence of the number of labeled objects on the dimensionality is neccesary due to the lack of regularization in the procedure. If we would take too few labeled objects, the \emph{supervised} estimator becomes unstable and subsequently the comparison to the projection of this estimator is hard to interpret. The results are not sensitive to the choice of $i$ for small values of $i$. In our experiments we set $i=5$. 
